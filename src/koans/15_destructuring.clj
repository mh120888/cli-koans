...
(meditations
  ...
  (= "Rich Hickey aka The Clojurer aka Go Time aka Lambda Guru"
     (let [[first-name last-name & aliases]
       ...
       (clojure.string/join " aka " [(clojure.string/join " " [first-name last-name]) (clojure.string/join " aka " aliases)])))
  ...
  (= "Test Testerson, 123 Test Lane, Testerville, TX"
     ((fn [[first-name last-name] {:keys [street-address city state]}]
      (clojure.string/join ", " [(clojure.string/join " " [first-name last-name]) street-address city state]))
